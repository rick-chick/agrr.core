...
class TestPredictionModelGatewayImpl:
    ...
    def test_predict_with_default_model(self):
        ...
        mock_model = Mock()
        mock_model.get_required_data_days = Mock(return_value=30)
        mock_model.predict = Mock(return_value=create_sample_forecasts())
        
        gateway = PredictionModelGatewayImpl(arima_service=mock_model)
        historical_data = create_sample_weather_data(100)
        
        forecasts = gateway.predict(
            historical_data,
            metric='temperature',
            prediction_days=7
        )
    ...
    def test_predict_with_specific_model(self):
        ...
        mock_arima.predict = Mock(return_value=create_sample_forecasts())
        
        mock_lgb = Mock()
        mock_lgb.get_required_data_days = Mock(return_value=90)
        mock_lgb.predict = Mock(return_value=create_sample_forecasts())
    ...
    def test_predict_with_unavailable_model(self):
    ...
    def test_evaluate(self):
        ...
        mock_model = Mock()
        mock_model.evaluate = Mock(return_value={'mae': 1.5, 'rmse': 2.0})
    ...
    def test_get_model_info_all(self):
        ...
        info_arima = gateway.get_model_info('arima')
        info_lgb = gateway.get_model_info('lightgbm')
    ...
    def test_predict_ensemble_equal_weights(self):
        ...
        mock_arima.predict = Mock(return_value=arima_forecasts)
        ...
        mock_lgb.predict = Mock(return_value=lgb_forecasts)
        ...
        ensemble_forecasts = gateway.predict_ensemble(
            historical_data,
            metric='temperature',
            prediction_days=7,
