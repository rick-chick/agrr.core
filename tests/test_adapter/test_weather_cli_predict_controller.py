...
class TestWeatherCliPredictController:
    ...
    def test_handle_predict_file_command_success(self):
        """Test successful predict file command execution."""
        predictions = [
            Forecast(date=datetime(2024, 2, 1), predicted_value=17.0)
        ]
        self.controller.predict_interactor.execute = Mock(return_value=predictions)
        ...
        # Execute
        self.controller.handle_predict_command(args)
    ...
    def test_handle_predict_file_command_validation_error(self):
        """Test predict file command with validation error."""
        self.controller.predict_interactor.execute = Mock(
    ...
    def test_handle_predict_file_command_internal_error(self):
    ...
    def test_run_predict_file_command(self):
    ...
    def test_run_unknown_argument(self):
        """Test run method with unknown argument."""
        with pytest.raises(SystemExit):
            self.controller.run(['--unknown-arg', 'value'])