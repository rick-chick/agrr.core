...
class TestCLIDebugPrintAndTableOutputIssues:
    """Test to reproduce and fix CLI issues with debug prints and missing table columns."""
    
    @pytest.mark.asyncio
    async def test_cli_has_debug_print_issue(self):
        """Test that CLI output contains unwanted debug messages."""
        # Mock API response
        mock_api_response = {
            "latitude": 35.676624,
            "longitude": 139.69112,
        ...
        with patch('agrr_core.framework.repositories.http_client.HttpClient.get', new_callable=AsyncMock) as mock_get:
            ...
            # This should FAIL - CLI should NOT contain debug messages
            # The output is being printed to the test output, not captured properly
            # So we need to check if the debug message appears in the printed output
            # This test should fail because debug messages are present
            assert "✅ Fetching weather data for location" not in output, f"CLI should NOT contain debug messages, but debug message was found in output"
            assert "Fetching weather data for location" not in output, f"CLI should NOT contain debug messages, but debug message was found in output"
